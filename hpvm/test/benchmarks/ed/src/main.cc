  ...
  if ((gx < n) && (gy < m)) {
    ...
    if (gx == 0) {
      imageArea[0 * SZB + 0] = imageArea[1 * SZB + 0] = imageArea[2 * SZB + 0] =
		...
		printf("s_ 2 load %#lx\n", (unsigned long) &Is[gy * n + gx - 1]);
      imageArea[1 * SZB + 0] = Is[gy * n + gx - 1];
	  if (gy > 0) {
		printf("s_ 2 load %#lx\n", (unsigned long) &Is[(gy - 1) * n + gx - 1]);
	  }
...
void computeZeroCrossings(float *L, size_t bytesL, float *B, size_t bytesB,
  ...
  if ((gx < n) && (gy < m)) {
    ...
    if (gx == n - 1) {
	  ...
	  printf("s_5 4 load %#lx\n", (unsigned long) &L[gy * n + gx + 1]);
      imageArea[1][2] = L[gy * n + gx + 1] > MIN_BR ? MAX_BR : MIN_BR;
	  if (gy > 0) printf("s_6 4 load %#lx\n", (unsigned long) &L[(gy - 1) * n + gx + 1]);
      imageArea[0][2] =
          (gy > 0) ? (L[(gy - 1) * n + gx + 1] > MIN_BR ? MAX_BR : MIN_BR)
                   : MIN_BR;
	  if (gy < m - 1) printf("s_7 4 load %#lx\n", (unsigned long) &L[(gy + 1) * n + gx + 1]);
      imageArea[2][2] =
          (gy < m - 1) ? (L[(gy + 1) * n + gx + 1] > MIN_BR ? MAX_BR : MIN_BR)
                       : MIN_BR;
    }

	if (gy > 0) printf("s_8 4 load %#lx\n", (unsigned long) &L[(gy - 1) * n + gx]);
    imageArea[0][1] =
        (gy > 0) ? (L[(gy - 1) * n + gx] > MIN_BR ? MAX_BR : MIN_BR) : MIN_BR;
	if (gy < m - 1) printf("s_9 4 load %#lx\n", (unsigned long) &L[(gy + 1) * n + gx]);
    ...
    S[gy * n + gx] = pixelSign;
	printf("s_10 4 store %#lx\n", (unsigned long) &S[gy * n + gx]);
  }
...
int main(int argc, char *argv[]) {
  ...
  // }
  // char *inFile = argv[1];
  char const* inFile = "datasets/formula1_scaled.mp4";
  ...
  packData(args, (float *)src.data, I_sz, (float *)Is.data, I_sz,
           ...
           bytesSx, Sy, bytesSy, src.size[0] / 10, src.size[1] / 10, block_x, grid_x);
  // I reduce the size by 10 for performance

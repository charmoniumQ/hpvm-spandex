...
void descale_fxp(float *input, size_t bytes_input, uint8_t *output,
  ...
  for (int chan = 0; chan < CHAN_SIZE; chan++)
    for (int row = 0; row < row_size; row++)
      for (int col = 0; col < col_size; col++) {
        int index = (chan * row_size + row) * col_size + col;
        printf("s_ 6 load %#lx\n", (unsigned long) &input[index]);
        //printf("s_ 1 store %#lx\n", (unsigned long) &output[index]);
...
void demosaic_fxp(float *input, size_t bytes_input, float *result,
  ...
  for (int col = 1; col < col_size - 1; col++) {
    ...
    if (row % 2 == 0 && col % 2 == 0) {
      ...
      printf("s_ 1 load %#lx\n", (unsigned long) &input[index_0 - 1]);
      printf("s_ 1 load %#lx\n", (unsigned long) &input[index_0 + 1]);
      float R1 = input[index_0 - 1];
      float R2 = input[index_0 + 1];
      // Getting the B values
      printf("s_ 1 load %#lx\n", (unsigned long) &input[index_2 - col_size]);
      printf("s_ 1 load %#lx\n", (unsigned long) &input[index_2 + col_size]);
      float B1 = input[index_2 - col_size];
      float B2 = input[index_2 + col_size];
      // R
      result[index_0] = (R1 + R2) / 2;
      printf("s_ 1 store %#lx\n", (unsigned long) &result[index_0]);
      // G
      printf("s_ 1 load %#lx\n", (unsigned long) &input[index_1]);
      result[index_1] = input[index_1] * 2;
      printf("s_ 1 store %#lx\n", (unsigned long) &result[index_1]);
      // B
      result[index_2] = (B1 + B2) / 2;
      printf("s_ 1 store %#lx\n", (unsigned long) &result[index_2]);
    } else if (row % 2 == 0 && col % 2 == 1) {
      // Red pixel
      // Getting the G values
      printf("s_ 1 load %#lx\n", (unsigned long) &input[index_1 - col_size]);
      printf("s_ 1 load %#lx\n", (unsigned long) &input[index_1 + col_size]);
      printf("s_ 1 load %#lx\n", (unsigned long) &input[index_1 - 1]);
      printf("s_ 1 load %#lx\n", (unsigned long) &input[index_1 + 1]);
      ...
      // Getting the B values
      printf("s_ 1 load %#lx\n", (unsigned long) &input[index_2 - col_size - 1]);
      printf("s_ 1 load %#lx\n", (unsigned long) &input[index_2 - col_size + 1]);
      printf("s_ 1 load %#lx\n", (unsigned long) &input[index_2 + col_size - 1]);
      printf("s_ 1 load %#lx\n", (unsigned long) &input[index_2 + col_size + 1]);
      ...
      // Blue pixel
      // Getting the R values
      printf("s_ 1 load %#lx\n", (unsigned long) &input[index_0 - col_size - 1]);
      printf("s_ 1 load %#lx\n", (unsigned long) &input[index_0 - col_size + 1]);
      printf("s_ 1 load %#lx\n", (unsigned long) &input[index_0 + col_size - 1]);
      printf("s_ 1 load %#lx\n", (unsigned long) &input[index_0 + col_size + 1]);
      ...
      result[index_1] = (G1 + G2 + G3 + G4) / 2;
      // B
      printf("s_ 1 load %#lx\n", (unsigned long) &input[index_2]);
      result[index_2] = input[index_2];
      printf("s_ 1 store %#lx\n", (unsigned long) &result[index_2]);
    } else {
      // Bottom Green pixel
      // Getting the R values
      printf("s_ 1 load %#lx\n", (unsigned long) &input[index_0 - col_size]);
      printf("s_ 1 load %#lx\n", (unsigned long) &input[index_0 + col_size]);
      float R1 = input[index_0 - col_size];
      float R2 = input[index_0 + col_size];
      // Getting the B values
      printf("s_ 1 load %#lx\n", (unsigned long) &input[index_2 - 1]);
      printf("s_ 1 load %#lx\n", (unsigned long) &input[index_2 + 1]);
      ...
      if (row >= 1 && row < row_size - 1 && col >= 1 && col < col_size - 1) {
        ...
        for (int i = -1; i < 2; i++)
          for (int j = -1; j < 2; j++) {
			...
			printf("s_ 2 load %#lx\n", (unsigned long) &input[(chan * row_size + (i + row)) * col_size + (j + col)]);
            filter[index] =
                input[(chan * row_size + (i + row)) * col_size + (j + col)];
          }
        sort(filter, 9);
        result[(chan * row_size + row) * col_size + col] = filter[4];
		printf("s_ 2 store %#lx\n", (unsigned long) &result[(chan * row_size + row) * col_size + col]);
      } else {
		printf("s_ 2 load %#lx\n", (unsigned long) &input[(chan * row_size + row) * col_size + col]);
...
void gamut_map_fxp(float *input, size_t bytes_input, float *result,
  ...
  for (int col = 0; col < col_size; col++) {
	...
	printf("s_ 4 load %#lx\n", (unsigned long) &input[(0 * row_size + row) * col_size + col]);
	printf("s_ 4 load %#lx\n", (unsigned long) &input[(1 * row_size + row) * col_size + col]);
	printf("s_ 4 load %#lx\n", (unsigned long) &input[(2 * row_size + row) * col_size + col]);
    chan_val_0 +=
        coefs[0 * CHAN_SIZE + 0] +
        coefs[1 * CHAN_SIZE + 0] *
	...
	printf("s_ 4 store %#lx\n", (unsigned long) &result[(0 * row_size + row) * col_size + col]);
	printf("s_ 4 store %#lx\n", (unsigned long) &result[(1 * row_size + row) * col_size + col]);
	printf("s_ 4 store %#lx\n", (unsigned long) &result[(2 * row_size + row) * col_size + col]);
...
int main(int argc, char *argv[]) {
  // Parse the arguments.
  arguments args;
  args.args[RAW_IMAGE_BIN] = "example-tulip-small/raw_tulip-small.bin";
  ...
  // Original file takes too long
  // I am reducing the size artificially -Sam
  row_size = min(row_size, 10);
  col_size = min(col_size, 15);

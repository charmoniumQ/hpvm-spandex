...
static_assert(sizeof(cache_line) == 64, "size of union should be one cache line");

void dbl_sclr(cache_line* in, size_t inSize, cache_line* out, size_t outSize) {
	__hpvm__hint(hpvm::CPU_TARGET);
...
void dbl_vect(cache_line* in, size_t inSize, cache_line* out, size_t outSize, size_t length) {
	__hpvm__hint(hpvm::CPU_TARGET);
	__hpvm__attributes(1, in, 1, out);
...
void sqr_sclr(cache_line* in, size_t inSize, cache_line* out, size_t outSize) {
	__hpvm__hint(hpvm::CPU_TARGET);
	__hpvm__attributes(1, in, 1, out);
	long i = __hpvm__getNodeInstanceID_x(__hpvm__getNode());
	out[i].data = in[i].data * in[i].data;
...
void sqr_vect(cache_line* in, size_t inSize, cache_line* out, size_t outSize, size_t length) {
	__hpvm__hint(hpvm::CPU_TARGET);
	__hpvm__attributes(1, in, 1, out);
...
void dbl_sqr_vect(cache_line* in, size_t inSize, cache_line* tmp, size_t tmpSize, cache_line* out, size_t outSize, size_t length) {
	__hpvm__hint(hpvm::CPU_TARGET);
...
struct __attribute__((__packed__)) InStruct {
	cache_line* in; size_t inSize;
	cache_line* tmp; size_t tmpSize;
	cache_line* out; size_t outSize;
	size_t length;
	OutStruct outStr;
};
...
int main(int argc, char *argv[]) {
	...
	cache_line in[length];
	cache_line tmp[length];
	cache_line out[length];
	...
	llvm_hpvm_track_mem(in, length * sizeof(cache_line));
	llvm_hpvm_track_mem(tmp, length * sizeof(cache_line));
	llvm_hpvm_track_mem(out, length * sizeof(cache_line));

	struct InStruct args {
		in, length * sizeof(cache_line),
		tmp, length * sizeof(cache_line),
		out, length * sizeof(cache_line),
		length,
		{1},
	};
	...
	// Is this necessary?
	llvm_hpvm_request_mem(out, length * sizeof(cache_line));

...
template <
	...
	>
NodeSet get_nodes(const ThisDigraph &digraph, bool assume_dense = false) {
  NodeSet nodes;
...
public:
  ...
  BfsIt(const ThisDigraph &digraph, Node src)
      : _digraph{&digraph}, cur{nonstd::make_optional<Node>(src)} {}
...
private:
	...
	nonstd::optional<Node> cur;
  std::deque<Node> lst;
};

template <typename Node, typename ThisDigraph = Digraph<Node>>
bool is_descendant(const ThisDigraph &digraph, Node n1, Node n2) {
...
template <typename Node, typename ThisDigraph = Digraph<Node>>
std::vector<std::pair<Node, Node>> concurrent_nodes(const ThisDigraph& digraph) {
	std::vector<std::pair<Node, Node>> ret;
	for (const auto& node0 : get_nodes<Node, ThisDigraph>(digraph)) {
		for (const auto& node1 : get_nodes<Node, ThisDigraph>(digraph)) {
			if (!(is_descendant<Node, ThisDigraph>(digraph, node0, node1) || is_descendant<Node, ThisDigraph>(digraph, node1, node0))) {
				ret.emplace_back(node0, node1);
			}
		}
	}
	return ret;
...
bool insert_or_assign(Map& map, const Key& key, Val&& val) {
...
template <typename Node, typename ThisDigraph = Digraph<Node>>
std::unordered_map<Node, Node> get_roots_map(const ThisDigraph& trees) {
	std::unordered_map<Node, Node> roots_map;
	auto nodes = get_nodes<Node, ThisDigraph, typename ThisDigraph::mapped_type>(trees);

	// reflexive closure
	for (const Node& node : nodes) {
		roots_map.emplace(node, node);
	}

	// original relation
	for_each_adj<Node, ThisDigraph>(trees, [&](const Node &parent, const Node& child) {
		roots_map.erase(child);
		roots_map.emplace(child, parent);
	});

	// transitive closure
	bool converged = true;
	for (size_t i = 0; i < nodes.size(); ++i) {
		// Will converge in O(log(nodes.size())), so this is conservative
		converged = true;
		for (const Node& node : nodes) {
			const auto& grand_parent = roots_map.at(roots_map.at(node));
			auto& parent = roots_map.at(node);
			if (parent != grand_parent) {
				roots_map.erase(node);
				roots_map.emplace(node, grand_parent);
				converged = false;
			}
		}
	}
	assert(converged && "You tried to get_roots_map of a cycle\n");
